...
class UserFriendshipsControllerTest < ActionController::TestCase
	context "#index" do
		...
		context "when logged in" do
			...
			should "display friend's names" do
				...
				assert_match /Active/i, response.body
			end
	...
	context "#create" do
		...
		context "when logged in" do
			...
			context "successfully" do
				should "create two user_friendship objects" do
					assert_difference 'UserFriendship.count', 2 do
						post :create, user_friendship: { friend_id: users(:jack).profile_name }
					end
				end
			end
			...
			context "with a friend id" do
				setup do
					post :create, user_friendship: { friend_id: users(:jack) }
				end

				should "assign a friend object" do
					assert assigns(:friend)
					assert_equal users(:jack), assigns(:friend)
				end
				...
				should "redirect to the profile page of the friend" do
					assert_response :redirect
					assert_redirected_to profile_path(users(:jack))
				end
	...
	context "#destroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:isaac))
				create(:accepted_user_friendship, friend: users(:isaac), user: @friend)
